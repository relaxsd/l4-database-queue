...
class QueueDatabaseQueueUnitTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $queue = $this->getMockBuilder('Relaxsd\Queue\DatabaseQueue')->setMethods(['getTime'])->setConstructorArgs([$database = m::mock('Illuminate\Database\Connection'), 'table', 'default'])->getMock();
    ...
    public function testDelayedPushProperlyPushesJobOntoDatabase()
    {
        $queue = $this->getMockBuilder(
            'Relaxsd\Queue\DatabaseQueue')->setMethods(
    ...
    public function testBulkBatchPushesOntoDatabase()
    {
        ...
        $queue = $this->getMockBuilder('Relaxsd\Queue\DatabaseQueue')->setMethods(['getTime', 'getAvailableAt'])->setConstructorArgs([$database, 'table', 'default'])->getMock();
